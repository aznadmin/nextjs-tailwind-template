      ...
      {menuItem.title === 'Product' ? (
        ...
        <div className="px-3 mb-2 flex flex-col gap-2">
          {menuItem.subMenu['navigation'].map((column: any) => (
            <div key={column.label} className="flex flex-col gap-3">
              {column.label !== 'Developers' && (
                <label className="text-muted text-xs uppercase tracking-widest font-mono mt-4">{column.label}</label>
              )}
          ...
          <div className="flex flex-col py-2">
            <label className="text-muted text-xs uppercase tracking-widest font-mono">Troubleshooting</label>
            <TextLink
              hasChevron={false}
